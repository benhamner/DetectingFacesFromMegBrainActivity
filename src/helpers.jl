...
function evaluate_subject(subject)
    ...
    split = split_train_test(features, vec(y), seed=1)
...
function run_models(x_train, y_train, x_test, y_test, num_features)
    ...
    net    = fit(x_train[:,fea], y_train, neural_net_options(hidden_layers=[10], stop_criteria=StopAfterIteration(2000), display=true, track_cost=true))
